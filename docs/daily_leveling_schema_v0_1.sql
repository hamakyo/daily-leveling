-- Daily Leveling PostgreSQL DDL Draft v0.1
-- Target: PostgreSQL 15+
-- Notes:
-- - UUID generation uses pgcrypto.gen_random_uuid()
-- - Habit deletion is logically handled via is_active = false
-- - Session token is stored as a hash, never plaintext
-- - Weekday convention: 1=Mon ... 7=Sun
...
CREATE TABLE habits (
  ...
  CONSTRAINT chk_habits_target_weekdays_range
    CHECK (
      target_weekdays IS NULL
      OR NOT EXISTS (
        SELECT 1
        FROM unnest(target_weekdays) AS d
        WHERE d < 1 OR d > 7
      )
    )
);
...
-- habit_logs.user_id must match habits.user_id
-- =========================================================
...
-- =========================================================
-- Helpful view: currently valid sessions
-- =========================================================
