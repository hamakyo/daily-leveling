...
超過時は `429 RATE_LIMITED` と以下の header を返します。
...
通常応答でも同じ `RateLimit-*` header を返し、client や運用確認から現在の残量を追えるようにします。

## Auth Observability

auth 異常系は token 本文や secret を残さず、構造化 security log として記録します。

主な event:
- `auth_rate_limited`
- `google_jwks_fetch_failed`
- `google_jwks_cache_fallback_used`
- `google_id_token_invalid_alg`
- `google_id_token_missing_kid`
- `google_id_token_unknown_kid`
- `google_jwk_invalid_alg`
- `google_id_token_invalid_signature`

log には `event`, `timestamp`, `route`, `requestId`, `clientIp` などの最小限の運用情報だけを残します。
production では `pnpm exec wrangler tail --env production` で確認します。
